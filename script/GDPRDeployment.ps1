...
(
	...
	[Parameter(Mandatory = $true, HelpMessage="Optional tenant administration credentials")]
	[PSCredential]
	$Credentials
)
...
try
{
    ...
    If ($disclaimer -eq 1) { 
     ...
     #   Write-Host "Creating artifacts on target site" $GroupSiteUrl
       ...
       # Add-PnPSiteCollectionAppCatalog -site https://m365x307051.sharepoint.com/sites/GDPRManagement
